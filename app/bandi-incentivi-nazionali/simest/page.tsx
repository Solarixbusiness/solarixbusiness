...
// Server Components - import diretti per performance ottimali
...
import ErogazioniSection from './components/ErogazioniSection';
import ConsolidamentoSection from './components/ConsolidamentoSection';
import ObbrighiSection from './components/ObbrighiSection';
import CumulabilitaSection from './components/CumulabilitaSection';
import RevocaSection from './components/RevocaSection';
import ConseguenzeRevocaSection from './components/ConseguenzeRevocaSection';
import ProceduraRevocaSection from './components/ProceduraRevocaSection';
import ControlliSection from './components/ControlliSection';
import ComunicazioniSection from './components/ComunicazioniSection';
import StipulaSection from './components/StipulaSection';
...
export const metadata = {
  ...
  alternates: {
    canonical: 'https://solarixbusiness.it/bandi-incentivi-nazionali/simest',
  },
};
        ...
        <div className="bg-white rounded-lg shadow-lg p-6 mb-8">
          ...
          <div className="space-y-6">
            <div className="border-l-4 border-blue-500 pl-4">
              <h3 className="text-lg font-semibold text-blue-900 mb-3">Parte 1: Quadro Normativo e Definizioni (Sezioni 1-7)</h3>
              <div className="grid md:grid-cols-2 gap-2">
                {sections.slice(0, 7).map((section, index) => (
                  ...
                  >
                    <span className="font-medium text-blue-700">{index + 1}.</span> {section.title.replace(/^\d+\.\s*/, '')}
                  </a>
                ))}
              </div>
            </div>
            ...
            <div className="border-l-4 border-blue-500 pl-4">
              <h3 className="text-lg font-semibold text-blue-900 mb-3">Parte 2: Procedure e Gestione (Sezioni 8-16)</h3>
              <div className="grid md:grid-cols-2 gap-2">
                {sections.slice(7, 16).map((section, index) => (
                  <a
                    key={section.id}
                    href={`#${section.id}`}
                    className="text-left p-3 rounded-lg border border-gray-200 text-sm hover:bg-blue-50 hover:border-blue-300 transition-colors cursor-pointer block"
                  >
                    <span className="font-medium text-blue-700">{index + 8}.</span> {section.title.replace(/^\d+\.\s*/, '')}
                  </a>
                ))}
              </div>
            </div>
            ...
            <div className="border-l-4 border-blue-500 pl-4">
              <h3 className="text-lg font-semibold text-blue-900 mb-3">Parte 3: Controlli e Aspetti Finali (Sezioni 17-24)</h3>
              <div className="grid md:grid-cols-2 gap-2">
                {sections.slice(16).map((section, index) => (
                  <a
                    key={section.id}
                    href={`#${section.id}`}
                    className="text-left p-3 rounded-lg border border-gray-200 text-sm hover:bg-blue-50 hover:border-blue-300 transition-colors cursor-pointer block"
                  >
                    <span className="font-medium text-blue-700">{index + 17}.</span> {section.title.replace(/^\d+\.\s*/, '')}
                  </a>
                ))}
              </div>
            </div>
